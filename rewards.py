...
def make_AIRL_reward_fn(discriminator, global_session, env_reward_weight=0, entropy_weight=0.1, discriminator_reward_weight=1):
    def AIRL_reward(obs, next_obs, actions, action_log_probs, env_rewards, values, entropies):
        expert_probs = discriminator.expert_prob(obs, next_obs, action_log_probs, global_session)
