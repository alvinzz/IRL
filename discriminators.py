...
class BasisDiscriminator:
    def __init__(
        ...
        with tf.variable_scope(name):
            ...
            self.next_tasks_timesteps = self.tasks_timesteps[:, 1] + 1
            ...
            # self.basis = self.reward_network.layers['out']
            # ground truth
            points = tf.constant([[1, 0, 0], [0, 1, 0], [-1, 0, 0], [0, -1, 0]], dtype=tf.float32)
            self.basis = tf.norm(tf.reshape(tf.tile(self.obs, [4, 1]), (-1, 4, ob_dim)) - points, axis=2)
            self.next_basis = tf.norm(tf.reshape(tf.tile(self.next_obs, [4, 1]), (-1, 4, ob_dim)) - points, axis=2)

            # w(task, timestep)
            self.reward_weights = tf.get_variable('weights', [n_tasks, n_timesteps+1, basis_size], initializer=weight_init())
            self.value_weights = tf.get_variable('weights', [n_tasks, n_timesteps+1, basis_size], initializer=weight_init())

            # rewards
            self.this_batch_weights = tf.gather_nd(self.weights, self.tasks_timesteps)
            self.this_batch_next_weights = tf.gather_nd(self.weights, self.tasks_timesteps)
            self.rewards = tf.expand_dims(tf.reduce_sum(self.basis * tf.gather_ns(self.weights, ) - tf.reduce_sum(self.next_basis * self.weights, axis=3), axis=3)

            # estimate p(a | s, expert) as follows:
            self.experts_action_log_probs = self.rewards

            self.policies_action_log_probs = tf.placeholder(tf.float32, shape=[n_tasks, None, n_timesteps, 1], name='policies_action_log_probs')
            self.experts_log_probs = self.experts_action_log_probs - tf.reduce_logsumexp(tf.stack((self.experts_action_log_probs, self.policies_action_log_probs)), axis=0)

            # training
            self.labels = tf.placeholder(tf.float32, shape=[n_tasks, None, n_timesteps, 1], name='labels') # 1 if from expert else 0
            # self.experts_loss = -tf.log(self.experts_probs)
            # self.policies_loss = -tf.log(1-self.experts_probs)
            self.loss = -tf.reduce_mean(self.labels*self.experts_log_probs + (1-self.labels)*tf.log(1-tf.exp(self.experts_log_probs)+1e-8))
            self.train_op = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(self.loss)

    def expert_log_prob(self, task, obs, next_obs, policy_action_log_probs, global_session):
        expert_log_probs = global_session.run(
            self.experts_log_probs,
            feed_dict={self.obs: obs, self.next_obs: next_obs, self.policies_action_log_probs: policy_action_log_probs}
        )
        return expert_log_probs

    def reward(self, task, obs, global_session):
        rewards = global_session.run(
            self.rewards,
            feed_dict={self.obs: obs}
        )
    ...
    def train(self,
        experts_obs, experts_next_obs, experts_actions,
        policies_obs, policies_next_obs, policies_actions,
        policies, global_session,
        n_iters=50, batch_size=1,
    ):
        #experts_obs, etc. = np.array(size=(n_tasks, #_demos, n_timesteps, ob_dim))
        if self.last_loss < -np.log(0.9):
            print('increasing noise param to weaken discriminator')
            self.noise_param += 0.01
        else:
        ...
        experts_obs[:, :, :, :-1] += np.random.normal(loc=0, scale=self.noise_param*np.std(np.reshape(experts_obs, (-1, self.ob_dim)), axis=1), size=experts_obs[:, :, :, :-1].shape)
        experts_next_obs[:, :, :, :-1] += np.random.normal(loc=0, scale=self.noise_param*np.std(np.reshape(experts_next_obs, (-1, self.ob_dim)), axis=1), size=experts_next_obs[:, :, :, :-1].shape)
        experts_actions += np.random.normal(loc=0, scale=self.noise_param*np.std(np.reshape(experts_actions, (-1, self.ob_dim)), axis=1), size=experts_actions.shape)
        policies_obs[:, :, :, :-1] += np.random.normal(loc=0, scale=self.noise_param*np.std(np.reshape(policies_obs, (-1, self.ob_dim)), axis=1), size=policies_obs[:, :, :, :-1].shape)
        policies_next_obs[:, :, :, :-1] += np.random.normal(loc=0, scale=self.noise_param*np.std(np.reshape(policies_next_obs, (-1, self.ob_dim)), axis=1), size=policies_next_obs[:, :, :, :-1].shape)
        policies_actions += np.random.normal(loc=0, scale=self.noise_param*np.std(np.reshape(policies_actions, (-1, self.ob_dim)), axis=1), size=policies_actions.shape)

        policies_action_log_probs = []
        for task in range(self.n_tasks):
            policies_action_log_probs.append(
                np.reshape(
                    global_session.run(
                        policies[tasks].action_log_probs,
                        feed_dict={policies[task].obs: np.reshape(policies_obs[task], (-1, self.ob_dim)), policies[task].actions: np.reshape(policies_actions[task], (-1, self.ob_dim))}
                    ),
                    (-1, n_timesteps, ob_dim)
                )
            )
        policies_action_log_probs = np.expand_dims(policies_action_log_probs, axis=3)

        experts_action_log_probs_under_policy = []
        for task in range(self.n_tasks):
            experts_action_log_probs_under_policy.append(
                np.reshape(
                    global_session.run(
                        policies[tasks].action_log_probs,
                        feed_dict={policies[task].obs: np.reshape(experts_obs[task], (-1, self.ob_dim)), policies[task].actions: np.reshape(experts_actions[task], (-1, self.ob_dim))}
                    ),
                    (-1, n_timesteps, ob_dim)
                )
            )
        experts_action_log_probs_under_policy = np.expand_dims(experts_action_log_probs_under_policy, axis=3)

        labels = np.concatenate((np.ones((self.n_tasks, batch_size, self.n_timesteps, 1)), np.zeros((self.n_tasks, batch_size, self.n_timesteps, 1))))

        for iter_ in range(n_iters):
            mb_experts_obs, mb_experts_next_obs, mb_experts_action_log_probs_under_policy = sample_minibatch(experts_obs, experts_next_obs, experts_action_log_probs_under_policy, batch_size)
            mb_policies_obs, mb_policies_next_obs, mb_policies_action_log_probs = sample_minibatch(policies_obs, policies_next_obs, policies_action_log_probs, batch_size)
            mb_obs = np.concatenate((mb_experts_obs, mb_policies_obs))
            mb_next_obs = np.concatenate((mb_experts_next_obs, mb_policies_next_obs))
            mb_policies_action_log_probs = np.concatenate((mb_experts_action_log_probs_under_policy, mb_policies_action_log_probs))
            global_session.run(
                self.train_op,
                feed_dict={self.obs: mb_obs, self.next_obs: mb_next_obs, self.policies_action_log_probs: mb_policies_action_log_probs, self.labels: labels}
            )
...
if __name__ == '__main__':
    a = AIRLDiscriminator('discriminator', 4)
    b = BasisDiscriminator('test', 3)
