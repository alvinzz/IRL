...
class BasisDiscriminator:
    def __init__(
        ...
        with tf.variable_scope(name):
            self.obs = tf.placeholder(tf.float32, shape=[None, ob_dim], name='obs')
            self.next_obs = tf.placeholder(tf.float32, shape=[None, ob_dim], name='next_obs')
            # reward network. shared accross tasks and timesteps
            self.basis_network = MLP('basis', ob_dim, 1, out_activation=out_activation, hidden_dims=hidden_dims, hidden_activation=hidden_activation, weight_init=weight_init, bias_init=bias_init, in_layer=self.obs)
            self.rewards = self.reward_network.layers['out']
            # w(task, timestep)
            self.weights =
    ...
    def expert_prob(self, task, obs, next_obs, policy_action_log_probs, global_session):
        expert_probs = global_session.run(
            self.expert_probs,
            feed_dict={self.obs: obs, self.next_obs: next_obs, self.policy_action_log_probs: policy_action_log_probs}
        )
