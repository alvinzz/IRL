...
class SHAIRL:
    def __init__(self,
        ...
        with tf.variable_scope(name):
            ...
            self.experts_obs = experts_obs
            self.experts_next_obs = experts_next_obs
            self.experts_actions = experts_actions

            self.policy = GaussianMLPPolicy('policy', self.ob_dim, self.action_dim)
            self.discriminator = AIRLDiscriminator('discriminator', self.ob_dim-1) # last dimension is time
