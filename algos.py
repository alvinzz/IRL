...
class RL:
    def __init__(self,
        ...
        with tf.variable_scope(name):
            ...
            self.policy = GaussianMLPPolicy('policy', self.ob_dim, self.action_dim, hidden_dims=[64, 64, 64, 64], learn_vars=True)
...
class SHAIRL:
    def __init__(self,
        ...
        with tf.variable_scope(name):
            ...
            self.policies = [GaussianMLPPolicy('policy{}'.format(task), self.ob_dim+1, self.action_dim, hidden_dims=[64, 64, 64, 64], learn_vars=True) for task in range(self.n_tasks)]
            self.discriminator = SHAIRLDiscriminator('discriminator', self.ob_dim, self.action_dim, self.n_tasks, self.n_timesteps, basis_size, hidden_dims=[64, 64, 64])
        ...
        for iter_ in range(n_iters):
            ...
            if obs_buffer[0] is not None:
                self.discriminator.train(
                    self.expert_obs, self.expert_next_obs, self.expert_actions,
                    obs_buffer, next_obs_buffer, actions_buffer,
                    self.policies, self.sess, n_iters=10, min_loss=0.01, #min_loss=0.1*(n_iters - iter_)/n_iters
                )
            ...
            for task in range(self.n_tasks):
                ...
                i = 0
                while np.sum(rewards)/batch_timesteps < np.log(0.4) and i < 1:
